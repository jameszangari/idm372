...
module.exports = {
  ...
  search: function()
  {
    ...
    const searchForm = document.querySelector('#searchSong');
    searchForm.addEventListener('submit', (event) =>
    {
      ...
      { //receive info, populate html, add event listeners to tracks to add as anthem
        // Add tracks to page in search results
        results.forEach(result =>
        {
          if (result.id)
          { // Skip the empty results (Idk why Spotify returns those...)
            const id = result.id,
              thumb = result.thumb,
              title = result.title,
